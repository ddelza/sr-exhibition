...
- **키를 evaluatorId로 둔 이유**: 같은 평가자가 폼을 다시 저장해도 새 자식이 늘어나지 않고
  ...
  쓸 것.
- 문항1(모둠원별 설명·잘한점·아쉬운점)은 대상 모둠원 **전원**에 대해 입력칸을 보여주되(발표
  당일 실제로 발표한 절반만 필터링하려 하지 말 것 — role 데이터가 그 시점에 완전하지 않을
  위험이 있어 오히려 깨지기 쉽다), "실제로 발표에 참여한 모둠원만 작성해주세요" 안내문으로
  해결한다. 안 쓴 칸은 그냥 빈 문자열로 저장되고 열람 페이지에서 "(내용 없음)"으로 보이거나
  통째로 걸러내면 된다.
